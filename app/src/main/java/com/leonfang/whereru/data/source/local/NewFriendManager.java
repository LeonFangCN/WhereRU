package com.leonfang.whereru.data.source.local;
...
import com.leonfang.whereru.data.source.local.dao.DaoMaster;
import com.leonfang.whereru.data.source.local.dao.DaoSession;
import com.leonfang.whereru.data.source.local.dao.NewFriendDao;
...
public class NewFriendManager {
    ...
    private NewFriend getNewFriend(String uid){
        NewFriendDao dao =  openReadableDb().getNewFriendDao();
        return dao.queryBuilder().where(NewFriendDao.Properties.Uid.eq(uid),NewFriendDao.Properties.Status.eq(Config.STATUS_VERIFY_NONE))
                .build().unique();
    }
